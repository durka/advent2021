...
: winning-board? ( board -- ? )
    ...
    [ winning-row? ] any?
    swap
    flip [ winning-row? ] any?
    or
;

: game-over? ( boards -- ? )
    [ length 1 = ] keep
    first winning-board? and
;
...
: run-bingo ( draws boards -- winner score )
    ! remove winning boards from previous run
    [ winning-board? ] reject

    ! perform draw (we assume there is a draw to do, if there are still active boards)
